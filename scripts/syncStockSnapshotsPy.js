...
const SCRIPT_PATH = path.join(__dirname, 'fetch_stock_snapshots.py');
...
export const syncStockSnapshotsPy = async () => {
  const attempts = [
...
if (process.argv[1] === __filename) {
  syncStockSnapshotsPy().catch((error) => {
    console.error('[stocks-py-sync] failed:', error);
    process.exitCode = 1;
  });
}
